...
// We check for existence and parse them to run correctly in the Canvas environment.
...
const App = () => {
    ...
    const getCollectionPath = useCallback((collectionName) => {
        ...
        // Uses the private path /artifacts/{appId}/users/{userId}/{collectionName}
        return `artifacts/${appId}/users/${userId}/${collectionName}`;
    }, [userId]);

    const initializeFirebase = useCallback(async () => {
        // Check if configuration is missing (e.g., local dev without keys)
        const isLocal = !firebaseConfig || Object.keys(firebaseConfig).length === 0;
        ...
        try {
            ...
            const currentUserId = userAuth.currentUser?.uid || crypto.randomUUID(); // Fallback to random ID if auth fails
            setDb(firestore);
    ...
    // Data loading/listener effects (App Configuration)
    useEffect(() => {
        ...
        return () => unsubscribe();
    }, [isAuthReady, db, userId, getCollectionPath]); // Dependencies removed for state objects to prevent infinite loop

    // Data loading/listener effects (Migrations List)
    useEffect(() => {
        if (!isAuthReady || !db || !userId || userId.startsWith('error-state') || isLocalDevRef.current) return;
    ...
    const saveConfig = useCallback((newAppDetails = appDetails, newArchitectureData = architectureData) => {
        ...
        setDoc(detailsDocRef, { appDetails: newAppDetails, architectureData: newArchitectureData, updatedAt: serverTimestamp() }, { merge: true })
            .then(() => {
                 // Optimization: Avoid showing success notification on every single drag/change
            })
            .catch(e => {
                console.error("Save error:", e);
                showNotification(`Failed to save configuration: ${e.message}`, 'error');
            });
    }, [db, userId, getCollectionPath, showNotification]); // Removed appDetails, architectureData from dependencies for stability
        ...
        const interval = setInterval(() => {
            migrations.forEach(migration => {
                if (migration.status !== 'Completed' && migration.status !== 'Failed') {
                    ...
                    // Stop the loop if we are at the end of the statuses array
                    if (currentIndex === statuses.length - 1) return;

                    // Move to the next status
                    const newStatus = statuses[currentIndex + 1];

                    // Check if the migration should fail randomly
                    if (newStatus === 'Completed' && Math.random() < 0.1) {
                        const docRef = doc(db, getCollectionPath('migrations'), migration.id);
                        updateDoc(docRef, { status: 'Failed' }).catch(e => console.error("Sim update error:", e));
                        return;
                    }
    ...
    const validateArchitecture = useCallback((nodes, connections, isSourcePhase) => {
        ...
        // Rule 1: Check for isolated components (Error)
        const isolatedNodes = nodes.filter(node =>
            !phaseConnections.some(c => c.sourceId === node.id || c.targetId === node.id)
        );

        // Rule 2: All components must have details filled out (Warning/Error based on severity)
        const nodesMissingDetails = nodes.filter(n => !n.isDetailed);
        if (nodesMissingDetails.length > 0) warnings++;

        // Only count component isolation as an error if there are connections present
        if (isolatedNodes.length > 0 && nodes.length > 1) errors++;

        return { warnings, errors, isComplete: (warnings === 0 && errors === 0) && nodes.length > 0, isolatedNodes };
    }, []);
    ...
    const AppDetailsPage = () => {
        // Handle local state update and navigation
        const handleContinue = () => {
            if (!appDetails.appName) {
                showNotification('Application Name is required.', 'warning');
                return;
            }
            // Save config before navigating away (optional, since it auto-saves)
            saveConfig(appDetails, architectureData);
    ...
    const ArchitecturePlanner = () => {
        ...
        const handleDrop = (e, targetType) => {
            ...
            const updatedArchitectureData = {
                ...architectureData,
                [activeSetKey]: [...architectureData[activeSetKey], newNode],
                nextComponentId: architectureData.nextComponentId + 1,
            };

            setArchitectureData(updatedArchitectureData);
            saveConfig(appDetails, updatedArchitectureData); // Save immediately
        ...
        // --- DELETE NODE HANDLER ---
        const handleDeleteNode = (node, isSource) => {
            const nodeKey = isSource ? 'sourceNodes' : 'targetNodes';

            const newArchitectureData = {
                ...architectureData,
                // Filter out the node
                [nodeKey]: architectureData[nodeKey].filter(n => n.id !== node.id),
                // Filter out connections involving the node
                connections: architectureData.connections.filter(c => c.sourceId !== node.id && c.targetId !== node.id)
            };

            setArchitectureData(newArchitectureData);
            saveConfig(appDetails, newArchitectureData);
            showNotification(`Component ${node.name} removed.`, 'info');
        };
        ...
        const handleNodeClick = (node, isSource) => {
            ...
            if (startNodeId !== null) {
                ...
                const newConnections = [...architectureData.connections, {
                ...
                const updatedArchitectureData = { ...architectureData, connections: newConnections };
                setArchitectureData(updatedArchitectureData);
                saveConfig(appDetails, updatedArchitectureData);
                setConnectionContext({ nodeId: null, isSource: null });
        ...
        const renderConnections = (isSource) => {
            ...
            return phaseConnections.map(c => {
                ...
                const isSelectedConnection = connectionContext.nodeId === c.sourceId || connectionContext.nodeId === c.targetId;

                return (
                    <line
                        key={c.id}
                        x1={x1} y1={y1} x2={x2} y2={y2}
                        stroke={isSelectedConnection ? "#A78BFA" : "#4F46E5"} // Tailwind purple-400 or indigo-600
                        strokeWidth={isSelectedConnection ? "4" : "3"}
                        strokeLinecap="round"
        ...
        const NodeComponent = ({ node, isSource }) => {
            ...
            return (
                ...
                >
                    ...
                    <div className="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 p-0.5 bg-white rounded-full border border-gray-200 shadow-sm">
                        {statusIcon}
                    </div>
                    {isIsolated && <AlertTriangle className="absolute bottom-0 left-0 text-red-500 w-4 h-4" title="Isolated Component: Needs Connection" />}
                     {/* Delete Button */}
                    <button
                        onClick={(e) => { e.stopPropagation(); handleDeleteNode(node, isSource); }}
                        className="absolute bottom-0 right-0 transform translate-x-1/4 translate-y-1/4 p-0.5 bg-red-500 text-white rounded-full hover:bg-red-600 transition-colors shadow-md"
                    ...
                    <div className="col-span-12 md:col-span-9 lg:col-span-10 flex flex-col space-y-4">
                        ...
                        <div className="mt-4 flex justify-end">
                            <button
                                ...
                                disabled={!sourceGuidance.isComplete || !targetGuidance.isComplete}
                            >
    ...
    const ComponentDetailsModal = ({ node, isSource, closeModal, updateNode, showNotification }) => {
        ...
        const handleSave = () => {
            ...
            const isDetailed = checklist.every(([attr, , type]) => {
                ...
                if (type === 'checkbox') {
                    ...
                    // Numbers must be set, parsable, and not just an empty string
            ...
            updateNode(updatedNode); // Updates local state and triggers saveConfig
            closeModal();
        ...
        const handleDetailChange = (attr, value, type) => {
            if (type === 'checkbox') {
                ...
                // Allows users to type in the number field, even if partial/empty string
                setTempDetails(prev => ({ ...prev, [attr]: value }));
            } else {
                setTempDetails(prev => ({ ...prev, [attr]: value }));
            }
                ...
                <div className="space-y-4">
                    {migrations.map(migration => {
                        ...
                        return (
                            <div
                                key={migration.id}
                                className={`bg-white p-5 rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between transition-shadow hover:shadow-lg
                                  border-l-4 ${migration.status === 'Completed' ? 'border-green-500' : migration.status === 'Failed' ? 'border-red-500' : 'border-indigo-500'}
                                `}
                            >
                                <div className="flex items-start w-full sm:w-1/2 mb-3 sm:mb-0">
                                    <div className={`p-3 rounded-full mr-4 ${INFRA_COMPONENTS[migration.sourceComponentId]?.color || 'bg-blue-600'}`}>
        ...
        <div className="flex min-h-screen bg-gray-100 font-sans">
            ...
            >
                ...
                <div className={`mt-auto pt-4 border-t border-gray-200 ${isSidebarOpen ? 'block' : 'hidden'}`}>
                    ...
                    <button
                        onClick={() => saveConfig(appDetails, architectureData)}
